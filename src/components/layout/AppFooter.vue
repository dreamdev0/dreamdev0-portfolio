    ...
    <div class="flex w-full items-center gap-3 sm:gap-6 md:gap-8">
      ...
      <div ref="dotsRef" class="hidden flex-1 items-center gap-3 sm:gap-4 md:flex">
        <div class="bg-ctp-surface0/40 relative h-[3px] flex-1 overflow-hidden rounded-full">
          <div
            class="from-accent to-accent-strong absolute inset-y-0 left-0 bg-gradient-to-r transition-[width] duration-300 ease-out"
            :style="{ width: `${progress * 100}%` }"
          />
        </div>
        <div class="flex items-center gap-2.5">
          <button
            ...
            :title="label(s.key)"
            :disabled="!enabled"
            class="group h-2.5 w-2.5 rounded-full transition-all duration-300 disabled:opacity-40"
            :class="
              active === s.id
